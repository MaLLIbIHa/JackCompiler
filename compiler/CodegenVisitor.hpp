...
public:
	CodegenVisitor()
		: globalTable_(std::make_unique<SymbolTable>()),
		  ...
		  semantic_(std::make_unique<SemanticChecker>( 
                globalTable_.get(), localTable_.get()))
		{}
    ...
    void preVisit(ClassDec *cl) override {
        ...
        currentClass_ = std::static_pointer_cast<ClassSymbol>(globalTable_->find(className));
    }

    void preVisit(SubroutineDec *subDec) override {
        std::string subName = subDec->getName();
        currentSubrtn_ = std::static_pointer_cast<SubroutineSymbol>(currentClass_->findMember(subName));
        ...
        if (subKind == SubroutineKind::CONSTRUCTOR_S ||
            ...
            auto thisSym = std::make_shared<ArgumentVariableSymbol>("this",
                                                       currentClass_->getClassType(),
                                                       subDec->getLinePos(),
                                                       subDec->getInLinePos());
            if (subKind == SubroutineKind::CONSTRUCTOR_S) {
    ...
    void preVisit(VariableDecList* varList) override {
        for (unsigned i = 0; i < varList->children(); i++) {
            ...
            VarSymbol varSym(var->getVarName(),
                             var->getVarType(),
                             var->getLinePos(),
                             var->getInLinePos());
            localTable_->insert(var->getVarName(), std::make_shared<VarSymbol>(varSym));
        }
    }
    ...
    void postVisit(NameExpr* nameExpr) override {
        ...
        if (evalType) valueCat = ValueCategory::LVALUE;
	...
	void postVisit(NewArrayExpr* newArrExpr) override {
		...
		exprStack_.pop();
		semantic_->checkNewArray(elementType,
								 newArraySizeExpr.evalType,
								 newArrExpr);
	...
	void postVisit(DeleteArrayExpr* delArrExpr) override {
		ExprInfo newArraySizeExpr = exprStack_.top();
		exprStack_.pop();
		semantic_->checkDeleteArray(newArraySizeExpr.evalType,
									delArrExpr);
	}

    void postVisit(LiteralExpr *litExpr) override {
        exprStack_.push({litExpr->getLiteralType(),
                         nullptr, 
                         ValueCategory::RVALUE});
    }
    ...
    void postVisit(ArrayMemberExpr *arrayExpr) override {
        ...
        ExprInfo nameInfo = exprStack_.top();
        exprStack_.pop();
		semantic_->checkArrayMember(nameInfo.evalType, indexInfo.evalType, arrayExpr);

        exprStack_.push({nameInfo.evalType->getArrayElementType(),
                         nullptr, ValueCategory::LVALUE});
    }
    ...
    void postVisit(SubroutineCallExpr *subCall) override {
        ...
        ExprInfo nameInfo = exprStack_.top();
        exprStack_.pop();
		SubroutineSymbol* subSym = semantic_->checkSubroutineCall(nameInfo.sym,
																	 subCall,
																	 argTypes.begin(),
																	 argTypes.end());
    ...
    void postVisit(LetStatement *letStmt) override {
        ...
        exprStack_.pop();
		semantic_->checkLetStatement(lhs.valCat, lhs.evalType,
									  rhs.evalType, letStmt);
    }
