...
public:
	symbolsVisitor(SymbolTable *globalTable) : 
        currentClass_(std::make_shared<ClassSymbol>()), globalTable_(globalTable) {}

    void preVisit(ClassDec *cl) override {
        currentClass_->setName(cl->getName());
        currentClass_->setLinePos(cl->getLinePos());
        currentClass_->setInLinePos(cl->getInLinePos());
    }

    void preVisit(SubroutineDec *subDec) override {
        std::shared_ptr<Node> argsList = subDec->child(0);
        std::vector<Type*> argTypes;
        for (unsigned i = 0; i < argsList->children(); i++) {
            auto arg = std::static_pointer_cast<VariableDec>(argsList->child(i));
            Type* type = arg->getVarType();
            argTypes.push_back(type);
        }

        std::string subName = subDec->getName();
        SubroutineSymbol subSym(subName,
                                subDec->getRetType(),
                                argTypes,
                                subDec->getSubroutineKind(),
                                subDec->getLinePos(),
                                subDec->getInLinePos());
        currentClass_->addSymbol(subName, std::make_shared<SubroutineSymbol>(subSym));
    }
    ...
    void preVisit(FieldVariableList *memberList) override {
        for (unsigned idx = 0; idx < memberList->children(); idx++) {
            ...
            FieldVariableSymbol varSym(var->getVarName(),
                            var->getVarType(),
                            currentClass_.get(),
                            idx,
                            var->getLinePos(),
                            var->getInLinePos());
            currentClass_->addSymbol(
                var->getVarName(), std::make_shared<FieldVariableSymbol>(varSym));
        }
    }
    ...
    void preVisit(StaticVariableList *memberList) override {
        for (unsigned idx = 0; idx < memberList->children(); idx++) {
            auto var = std::static_pointer_cast<VariableDec>(memberList->child(idx));
            StaticVariableSymbol varSym(var->getVarName(),
                                var->getVarType(),
                                currentClass_.get(),
                                var->getLinePos(),
                                var->getInLinePos());
            currentClass_->addSymbol(
                var->getVarName(), std::make_shared<StaticVariableSymbol>(varSym));
        }
    }
